...
def get_mail_handler(name="app", level=DEF_LEVEL):
    ...
    handler = GmailSMTPHandler(
            ...
            Settings.MAIL_TO,
            "Server Error.",
            (Settings.MAIL_FROM, Settings.MAIL_PASSWORD))
    handler.setLevel(level)
...
# Already created a default logger.
DEFAULT_LOGGER = get_file_vomitter(name="default", level=logging.DEBUG)
PROD_LOGGER = get_mail_vomitter(name="Mailer", level=logging.DEBUG)
